...
void used_reserved(char *s) {
	fprintf(stderr, "%s: e - line %d of \"%s\", illegal use of "
...
void tokenized_start(char *s) {
	fprintf(stderr, "%s: e - line %d of \"%s\", the start symbol %s "
	        "cannot be declared to be a token\n", myname, lineno,
	        input_file_name, s);
	done(1);
}

void retyped_warning(char *s) {
	fprintf(stderr, "%s: w - line %d of \"%s\", the type of %s has "
	        "been redeclared\n", myname, lineno, input_file_name, s);
}
